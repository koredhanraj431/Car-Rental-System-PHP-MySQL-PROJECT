...
<body>
 ...
 Dear $username,  
    ...
    If you have made Payment, Your refund will be initiated within 10-15 buisness days.
   ...
   Drive your Dreams";
    ...
    if (isset($_POST['reject'])) {
        ...
        //update car status
        $qr_carid = "SELECT carid FROM clients WHERE id='$id'";
        $car_res = $dbh->query($qr_carid);
        $car_rws = $car_res->fetch_assoc();
        $carr_id=$car_rws['carid'];

        $updtcar = "UPDATE car SET status = 'Available' WHERE car_id='$carr_id'";
        mysqli_query($dbh, $qr);
   ...
   Drive your Dreams";
    ...
    <div class="container bookings">
        <div class="items">
            ...
            <div style="margin-top: 20px;">
                <div>
                    ...
                    <p>Booking Date & Time : <?php echo $rws1['booking_date']; ?></p>
                    <p>Client Username : <?php echo $rws1['username']; ?></p>
                    <p>Booked Car ID : <?php echo $rws1['carid']; ?></p>
                    <p>Pick-Up Date : <?php echo $rws1['pick_date']; ?></p>
                    <p>Drop Off Date : <?php echo $rws1['drop_date']; ?></p>
        ...
        <div class="items">
            ...
            <?php if ($rws1['status'] == 'Paid' || $rws1['status'] == 'Approved' || $rws1['status'] == 'Rejected') {
                $pay = "SELECT * FROM payment WHERE id='$b_id'";
                $payres = $dbh->query($pay);
                $payrws = $payres->fetch_assoc();
            ?>
                <div style="margin-top: 250px;">
                    <h3>Payment Details : </h3>
                    <div style="margin-top: 10px;">
                        <p>Mode of Payment : Debit Card</p>
                        <p>Card Number : <?php echo $payrws['card_no']; ?></p>
                        <p>Name on Card : <?php echo $payrws['name']; ?></p>
                        <p>Card Expiry-date: <?php echo $payrws['expiry_date']; ?></p>
                        <p>Postal Code : <?php echo $payrws['zip']; ?></p>
                    </div>
                </div>
            <?php } ?>
        </div>
