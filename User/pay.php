...
<body>
	...
	if (isset($_POST['cont_booking'])) {
		$id = $_POST['cont_booking'];
		$_SESSION['id'] = "$id";
		$qr = "SELECT * FROM clients WHERE id='$id'";
		$res = $db->query($qr);
		$rws = $res->fetch_assoc();
		$amount = $rws['amount'];
	}
	...
	if (isset($_POST['pay'])) {
		...
		$qr1 = "INSERT INTO payment (id,name, card_no, expiry_date, zip) VALUES('$id','$name', '$cno', '$exp','$zip')";
		mysqli_query($db, $qr1);

		$upd = "UPDATE clients SET status='Paid' WHERE id='$id'";
		mysqli_query($db, $upd);
 ...
 Dear $username,  
   Congratulations! Your Booking is successful.
   
   Thank you for booking with India's leading and one stop Car rental platform. Drive your Dreams!

   Your Booking ID is $id. Note this Id for future references
   
   You will receive confirmation from us shortly!

   Thanks & Regards!

   Drive your Dreams";

		$sender = "From: techwebbeast99@gmail.com";
		...
		<div class="alert-box">
			<div class="alert alert-success">
				...
				<div class="alert-message text-center">
					<h5>Car details registered successfully!</h5>
					<strong>Please make your payment to complete your registration.</strong>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="container" style="margin-top: 20px; margin-bottom: 30px;">
		<div id="Checkout" class="inline">
			...
			<hr>
			<div class="Total">
				<h4 class="ml-4 mt-3 mb-3" style="color:brown; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;">
					Total Amount to Pay : <p style="float:right; margin-right: 10px"><?php echo $amount; ?>/-</p>
				</h4>
			</div>
			<hr>
			<form method="POST" action="">
				<div class="form-group">
					...
					<input id="NameOnCard" class="form-control" type="text" name="name" maxlength="255" required></input>
				</div>
				<div class="form-group">
					<label for="CreditCardNumber">Card number</label>
					<input id="CreditCardNumber" name="card" class="null card-image form-control" type="number" required></input>
				</div>
				<div class="expiry-date-group form-group">
					<label for="ExpiryDate">Expiry date</label>
					<input id="ExpiryDate" class="form-control" name="exp" type="text" placeholder="MM / YY" maxlength="7" required></input>
				</div>
				<div class="security-code-group form-group">
					<label for="SecurityCode">CVV</label>
					<div class="input-container">
						<input id="SecurityCode" class="form-control" type="password" placeholder="3 digit number" required></input>
				...
				<div class="zip-code-group form-group">
					...
					<div class="input-container">
						<input id="ZIPCode" name="zip" class="form-control" type="text" maxlength="10" required></input>
						<a role="button" data-toggle="popover" data-trigger="focus" data-placement="left" data-content="Enter the ZIP/Postal code for your credit card billing address."><i class="fa fa-question-circle"></i></a>
					</div>
				</div>
				<button id="PayButton" name="pay" class="btn btn-block btn-success submit-button" type="submit">
